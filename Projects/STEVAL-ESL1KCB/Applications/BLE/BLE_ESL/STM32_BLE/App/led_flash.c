...
typedef struct 
{
  ...
  VTIMER_HandleType led_change_timer;
  VTIMER_HandleType duration_timer;
} LEDFlashState_t;
...
int LED_FLASH_Start(uint8_t led_index, uint8_t flashing_pattern[5], uint8_t off_period, uint8_t on_period, uint8_t repeat_type, uint16_t repeat_duration)
{
  ...
  /* Just in case timers are already started. */
  HAL_RADIO_TIMER_StopVirtualTimer(&led_flash_states[led_index].led_change_timer);
  led_flash_states[led_index].led_change_timer.callback = LEDChangeTimerCB;
  led_flash_states[led_index].led_change_timer.userData = &led_flash_states[led_index];
  
  HAL_RADIO_TIMER_StopVirtualTimer(&led_flash_states[led_index].duration_timer);
  led_flash_states[led_index].duration_timer.callback = durationTimerCB;
  led_flash_states[led_index].duration_timer.userData = &led_flash_states[led_index];
  ...
  {
    HAL_RADIO_TIMER_StartVirtualTimer(&led_flash_states[led_index].duration_timer, repeat_duration*1000);
  }
...
static void programNextLEDChange(LEDFlashState_t *led_state_p)
{
  ...
  HAL_RADIO_TIMER_StartVirtualTimer(&led_state_p->led_change_timer, time_interval_ms);
}
...
void LEDChangeTimerCB(void *arg)
{
  ...
  LEDFlashState_t *led_state_p = timer_p->userData;
...
void durationTimerCB(void *arg)
{
  ...
  HAL_RADIO_TIMER_StopVirtualTimer(&led_state_p->led_change_timer);
}
