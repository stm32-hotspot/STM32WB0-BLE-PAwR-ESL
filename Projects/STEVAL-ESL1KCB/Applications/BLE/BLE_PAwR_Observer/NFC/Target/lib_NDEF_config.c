...
{
  if(NFCTAG_ReadData(NFCTAG_INSTANCE, pData, offset, length ) != NFCTAG_OK)
    return NDEF_ERROR;
  return NDEF_OK;
}
...
{
  if(NFCTAG_WriteData(NFCTAG_INSTANCE, pData, offset, length ) != NFCTAG_OK)
    return NDEF_ERROR;
  return NDEF_OK;
}
...
{
  return NFCTAG_GetByteSize(NFCTAG_INSTANCE);
}
