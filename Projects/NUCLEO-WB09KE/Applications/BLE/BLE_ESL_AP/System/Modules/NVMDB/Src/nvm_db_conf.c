...
#define FLASH_NVM_DATASIZE  (3 * PAGE_SIZE)  // Make sure this space is reserved in the linker script.
#define NVM_START_ADDRESS   (_MEMORY_FLASH_END_ - FLASH_NVM_DATASIZE + 1)

#define DB2_SIZE  (PAGE_SIZE)
...
{
  {
    ...
    .id = 2,
#if AUTO_CLEAN
    .clean_threshold = DB2_SIZE / 3
#endif
  },
  {
    .address = NVM_START_ADDRESS + DB2_SIZE,
    .size = DB0_SIZE,
    .id = 0,
#if AUTO_CLEAN
